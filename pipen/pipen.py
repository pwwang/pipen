...
from .defaults import DEFAULT_CONFIG, DEFAULT_CONFIG_FILES, config
from .exceptions import ProcDependencyError
from .plugin import PipenMainPlugin, plugin
from .proc import Proc, ProcType
from .progressbar import PipelinePBar
from .utils import (DEFAULT_CONSOLE_WIDTH, get_console_width,
                    get_plugin_context, log_rich_renderable, logger,
                    pipen_banner, render_scope)
...
class Pipen:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if Pipen.PIPELINE_COUNT == 0:
            ...
            config._load({"default": DEFAULT_CONFIG}, *DEFAULT_CONFIG_FILES)
    ...
    def _print_config(self) -> None:
        ...
        with context as conf:
            log_rich_renderable(
                render_scope(conf, "default configurations"), None, logger.info
            )
    ...
    def _init_procs(self, starts: List[ProcType]) -> int:
        ...
        self.procs = [
            start if isinstance(start, Proc) else start() for start in starts
        ]
