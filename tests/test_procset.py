...
def test_procset_delegate(ps3_copy_depends):
	...
	assert ps3_copy_depends.delegated('args.*') == [
		ps3_copy_depends.pProc1,
		ps3_copy_depends.pProc2,
...
def test_procset_copy(ps3_copy_depends):
	...
	assert copied.ends == []
	assert list(copied.procs.keys()) == [proc.id for proc in ps3_copy_depends.procs.values()]
...
def test_procset_module(ps3_copy_depends):
	...
	assert ps3_copy_depends.starts == [ps3_copy_depends.pProc1, ps3_copy_depends.pProc2]
	...
	assert ps3_copy_depends.starts == [
		ps3_copy_depends.pProc1, ps3_copy_depends.pProc2, ps3_copy_depends.pProc3]
