...
def test_theme_get_color():
    ...
    theme = Theme('test')
    assert theme.get_color('DONE') == THEMES['test'][get_group('DONE')].format(
        **Theme.COLORS)
    assert theme.get_color('INFO') == THEMES['test']['CRITICAL'].format(
        **Theme.COLORS)
    assert theme.get_color('ABCDEF') == ''
    assert theme.get_color(
        'ERROR') == THEMES['green_on_black']['ERROR'].format(**Theme.COLORS)
...
def test_stream_handler(capsys):
    ...
    record1 = logging.makeLogRecord(
        dict(msg="This is logging record1.",
             mylevel="INFO",
             proc='',
             ispbar=False,
             jobidx=None))
    ...
    record2 = logging.makeLogRecord(
        dict(msg="This is logging record2.",
             mylevel="INFO",
             proc='pProc',
             ispbar=True,
             done=True,
    ...
    record3 = logging.makeLogRecord(
        dict(msg="This is logging record3.",
    ...
    record4 = logging.makeLogRecord(
        dict(msg="This is logging record4.",
             mylevel="INFO",
             proc='pProc',
             ispbar=False,
             tails=[record3]))
...
def test_logger_init_levels():
    ...
    assert init_levels(True, []).issuperset({
        'PROCESS', 'DONE', 'DEPENDS', 'WORKDIR', 'CACHED', 'P_DONE', 'INFO',
        'CONFIG', 'PLUGIN', 'PYPPL', 'TIPS', 'BLDING', 'SBMTING', 'RUNNING',
        'JOBDONE', 'KILLING', 'RTRYING', 'ERROR', 'WARNING'
    })
    assert init_levels('DEBUG', []).issuperset({
        'BLDING', 'CACHED', 'DEBUG', 'DEPENDS', 'ERROR', 'INFO', 'JOBDONE',
        'KILLING', 'PROCESS', 'P_DONE', 'RTRYING', 'RUNNING', 'SBMTING',
        'WARNING', 'WORKDIR', 'DONE', 'CONFIG', 'PLUGIN', 'PYPPL', 'TIPS'
    })
    assert init_levels('CRITICAL', ['+DEBUG', '-WORKDIR']).issuperset({
        'BLDING', 'CACHED', 'DEBUG', 'DEPENDS', 'INFO', 'JOBDONE', 'KILLING',
        'PROCESS', 'P_DONE', 'RUNNING', 'SBMTING', 'DONE'
    })
