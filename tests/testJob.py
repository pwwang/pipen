...
class TestJob(testly.TestCase):
	...
	def dataProvider_testInit(self):
		config = CONFIG.copy()
		config.workdir = path.join(self.testdir, 'pInit')
		yield 0, config
		yield 1, config
	...
	def dataProvider_testExportSingle(self):
		...
		# expart (glob)
		config10         = CONFIG.copy()
		config10.proc    = 'pExportSingle10'
		...
		# expart (outkey)
		config11         = CONFIG.copy()
		config11.proc    = 'pExportSingle11'
		...
		# expart (no matches)
		config12         = CONFIG.copy()
		config12.proc    = 'pExportSingle12'
		...
		config13         = CONFIG.copy()
		config13.proc    = 'pExportSingle13'
	...
	def testSucceed(self, jobrc, procrc, out, expect = '', createOfs = False):
		config         = CONFIG.copy()
		config.proc    = 'pSucceed'
		config.expect  = TemplateLiquid(expect)
