...
class _TemplateFilter(object):
    ...
    def prefix(var, orig=False, dot=-1):
        ...
        return path.join(path.dirname(var),
                         _TemplateFilter.filename(var, orig, dot))
    ...
    def R(var, ignoreintkey=True):
        ...
        if isinstance(var, (list, tuple, set)):
            return 'c({})'.format(','.join([_TemplateFilter.R(i)
                                            for i in var]))
    ...
    def render(var, data=None):
        ...
        if not engine:
            raise RuntimeError(
                "I don't know which template engine to use to render {}...".
                format(var[:10]))
...
def default_envs():
    return {
        ...
        _TemplateFilter.R,
        'Rvec':
        _TemplateFilter.R,  # will be deprecated!
        'Rlist':
        _TemplateFilter.Rlist,
        'realpath':
        path.realpath,
        'readlink':
        readlink,
        ...
        # /a/b/c[1].txt => c.txt
        'basename':
        _TemplateFilter.basename,
        'bn':
        _TemplateFilter.basename,
        'diot':
        _TemplateFilter.diot,
        'odiot':
        _TemplateFilter.odiot,
        'stem':
        _TemplateFilter.filename,
        'filename':
        _TemplateFilter.filename,
        'fn':
        _TemplateFilter.filename,
        # /a/b/c.d.e.txt => c
        'filename2':
        lambda var, orig=False, dot=1: _TemplateFilter.filename(
            var, orig, dot),
        'fn2':
        lambda var, orig=False, dot=1: _TemplateFilter.filename(
            var, orig, dot),
        # /a/b/c.txt => .txt
        'ext':
        lambda var: path.splitext(var)[1],
        'glob1':
        _TemplateFilter.glob1,
        # /a/b/c[1].txt => /a/b/c
        'prefix':
        _TemplateFilter.prefix,
        # /a/b/c.d.e.txt => /a/b/c
        'prefix2':
        lambda var, orig=False, dot=1: _TemplateFilter.prefix(var, orig, dot),
        'quote':
        lambda var: json.dumps(str(var)),
        'squote':
        repr,
        'json':
        json.dumps,
        'read':
        _TemplateFilter.read,
        'readlines':
        _TemplateFilter.readlines,
        'render':
        _TemplateFilter.render,
        # single quote of all elements of an array
        'asquote':
        lambda var: '''%s''' % (" ".join([json.dumps(str(e)) for e in var])),
...
class TestTemplateLiquid:
    ...
    @pytest.mark.parametrize(
		...
		""", {
            ...
            ('{{glob1(here, "test_template.py")}}', {
                'here': HERE
            }, str(Path(__file__).resolve())),
            ('{{glob1(here, "test_template_not_exists.py")}}', {
                'here': HERE
            }, '__NoNeXiStFiLe__'),
            ('{{glob1(here, "test_template.py", first = False)[0]}}', {
                'here': HERE
            }, str(Path(__file__).resolve())),
        ])
    ...
    def test_filename_no_ext(self, default_envs):
        assert TemplateLiquid('{{a|fn}}',
                              **default_envs).render({'a': 'abc'}) == "abc"
...
class TestTemplateJinja2:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ('{{basename(v9, v9b)}}{{bn(v9, v9b)}}', {
                'v9': '/a/b/c.txt',
                'v9b': True
            }, 'c.txtc.txt'),
            # 10
            ('{{basename(v10, v10b)}}{{bn(v10, v10b)}} {{ext(v10)}}', {
                'v10': '/a/b/c[1].txt',
                'v10b': True
            }, 'c[1].txtc[1].txt .txt'),
            ('{{filename(v11)}}{{fn(v11)}} {{prefix(v11)}}', {
                'v11': '/a/b/a.txt'
            }, 'aa /a/b/a'),
            ('{{filename(v12)}}{{fn(v12)}}', {
                'v12': '/a/b/[1]b.txt'
            }, 'bb'),
            ('{{filename(v13, v13b)}}{{fn(v13, v13b)}}', {
                'v13': '/a/b/d.txt',
                'v13b': True
            }, 'dd'),
            ('{{filename(v14, v14b)}}{{fn(v14, v14b)}}', {
                'v14': '/a/b/[1]c.txt',
                'v14b': True
            }, '[1]c[1]c'),
            # 15
            ...
            ('{% for var in varlist %}{{R(var)}}{% endfor %}', {
                'varlist': ['abc', 'True', 1, False]
            }, "'abc'TRUE1FALSE"),
            ('{% if bool(var3) %}1{% else %}0{% endif %}', {
                'var3': 'abc',
                'bool': bool
            }, '1'),
