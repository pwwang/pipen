...
THEMES = dict(green_on_black=dict(TITLE='{s.BRIGHT}{f.CYAN}',
                                  SUBTITLE='{f.MAGENTA}',
                                  STATUS='{f.YELLOW}',
                                  CRITICAL='{f.GREEN}',
                                  ERROR='{f.RED}',
                                  ...
                                  DEBUG='{s.DIM}{f.WHITE}',
                                  NOTSET=''),
              blue_on_black=dict(TITLE='{s.BRIGHT}{f.CYAN}',
                                 SUBTITLE='{f.GREEN}',
                                 STATUS='{f.YELLOW}',
                                 CRITICAL='{f.BLUE}',
                                 ERROR='{f.RED}',
                                 ...
                                 DEBUG='{s.DIM}{f.WHITE}',
                                 NOTSET=''),
              magenta_on_black=dict(TITLE='{s.BRIGHT}{f.GREEN}',
                                    SUBTITLE='{f.BLUE}',
                                    STATUS='{f.YELLOW}',
                                    CRITICAL='{f.MAGENTA}',
                                    ERROR='{f.RED}',
                                    ...
                                    DEBUG='{s.DIM}{f.WHITE}',
                                    NOTSET=''),
              green_on_white=dict(TITLE='{s.BRIGHT}{f.BLUE}',
                                  SUBTITLE='{f.MAGENTA}',
                                  STATUS='{f.YELLOW}',
                                  CRITICAL='{f.GREEN}',
                                  ERROR='{f.RED}',
                                  ...
                                  DEBUG='{s.DIM}{f.BLACK}',
                                  NOTSET=''),
              blue_on_white=dict(TITLE='{s.BRIGHT}{f.GREEN}',
                                 SUBTITLE='{f.MAGENTA}',
                                 STATUS='{f.YELLOW}',
                                 CRITICAL='{f.BLUE}',
                                 ERROR='{f.RED}',
                                 ...
                                 DEBUG='{s.DIM}{f.BLACK}',
                                 NOTSET=''),
              magenta_on_white=dict(TITLE='{s.BRIGHT}{f.BLUE}',
                                    SUBTITLE='{f.GREEN}',
                                    STATUS='{f.YELLOW}',
                                    CRITICAL='{f.MAGENTA}',
                                    ERROR='{f.RED}',
...
class StreamFilter(logging.Filter):  # pylint: disable=too-few-public-methods
    ...
    def filter(self, record): # pylint: disable=too-many-return-statements
        # logging is disabled
        if not self.levels:
            return False
        ...
        if proc not in self.subs:
            self.subs = {
                proc: dict(zip(SUBLEVELS.keys(), [0] * len(SUBLEVELS)))
            }
...
class FileFilter(StreamFilter):  # pylint: disable=too-few-public-methods
    ...
    def filter(self, record):
        if (record.ispbar
                and not record.done) or not hasattr(record, 'formatted'):
            return False
...
def init_levels(group, leveldiffs):
    ...
    groups = [key for key in GROUP_VALUES if GROUP_VALUES[key] >= value]
...
class Logger:
    ...
    def add_level(self, level, group='INFO'):  # pylint: disable=no-self-use
        ...
        if group not in LEVEL_GROUPS:
            raise ValueError(
                'No such level group: {}, available ones are: {}'.format(
