...
class proc (object):
	...
	@staticmethod
	def registerRunner (runner_name):
		if not proc.runners.has_key(runner_name):
			proc.runners[runner_name] = getattr(__import__('runners.runner_%s' % (runner_name), fromlist=['runner_%s' % runner_name]), 'runner_%s' % runner_name)
			
proc.registerRunner ('local')
proc.registerRunner ('sge')
proc.registerRunner ('ssh')
