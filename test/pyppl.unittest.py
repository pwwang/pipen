...
""".split("\n")))
	...
	def test_batchjobs (self):
		p = proc ('batch')
		p.input = {'input': channel([1, 1, 1, 1, 1, 1])}
		p.script = "sleep {input}; hostname"
		p.echo = True
		p.forks = 3
		p.runner = 'ssh'
		pyppl({
			'proc': {
				'sshRunner': {	
					'servers': ['franklin01', 'franklin02', 'franklin03', 'franklin04', 'franklin05', 'franklin06', 'franklin07', 'franklin08', 'franklin09']
				}
			}
		}).starts(p).run()
