...
| `ppldir` | The directory to store `<workdir>s` for all processes in this pipeline | `str` | `"./workdir"`|~~tmpdir~~ (deprecated)|[Link][7]|
...
# p.args.bedtools = "/path/to/bedtools"
p.script = """
{{args.bedtools}} intersect -a {{infile1}} -b {{infile2}} > {{outfile}}
"""
```
...
> When you specify new dependents for a process, its original ones will be removed, which means each time `p.depends` will overwrite the previous setting.
...
pSingle.callback = lambda p: p.channel.collapse()
...
# generate bam files
# ...
pBam.output = "bamfile:file:{{infile | fn}}.bam"

# generate/download reference file, and index it
# ...
pRef.output = "reffile:file:{{in}}.fa"
