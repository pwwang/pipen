...
To configure it for a pipeline for all processes:
```python
pyppl ({
    'proc': {
...
To set them for a pipeline for all processes:
```python
pyppl ({
    'proc': {
        'sgeRunner': {
...
The base class `runner` defines the runners where the jobs will immediately run after submission; while `runner_queue` defines the runners where the jobs will be put into a queue and wait for its turn to run (for example, clusters).
...
> `runner` has already done pretty much all the jobs for the submission if you have the right `self.script`. You don't to rewrite the function, however, you can also do it if you want to add more actions there.
...
| `None` | `runner_queue` | Main thread dead, jobs alive | ~~Use `self.isRunning()` to tell whether jobs are truly alive. If yes, wait; otherwise quit.~~ (Abandoned, uses too much local resources). Nothing to do, jobs are still running. | `.stdout/.stderr` files |  
| `Not None` | `runner` | All alive | `p.wait()` | `.stdout`/`.stderr` files |  
| `Not None` | `runner_queue` | All alive | ~~Use `self.isRunning()` to tell whether jobs are truly alive. If yes, wait; otherwise quit~~ (Abandoned, uses too much local resources). Polling until return code is changed. | `.stdout/.stderr` files |
...
**Key points in writing your own runner**:
  1. Choose the right base class (`pyppl.runner` or `pyppl.runner_queue`) (required).
  ...
  5. Set the static value for `maxsubmit` and `interval` if necessary (only for `runner_queue`) (optional).
...
## Return code in `pyppl` and the actual number in `job.rc` file
When you try to set return code using `job.rc(val)`, it will basically write the number to `job.rc`. If `rc < 0` (output file is not generated or expectation failed, in this case) and the script returns `abs(rc)`. If the script returns 0, but output files are not generated, a warning of return code `-0` will be shown, but in `job.rc` file, the content is `-1000`
the number in `job.rc` is positive, then make it negative; if it is negative, `job.failedRc`, or `job.emptyRc`, keep it unchanged. When try to get the return code from the file, just get the number from the file. When the file does not exist or is empty, return `job.emptyRc`. If any of the output files are not generated or expectations not reached, the number will just become negative, you will get a negative number using `job.rc()`. So this makes sure a negative number from `job.rc()` (`-0` from the file will return `-1000`) meaning output file not generated or expectations not reached.


| `job.rc()` | Content of `job.rc` file | Value in log | Script return code | Meaning |
|------------|-------------------------|--------------|--------------------|---------|
| `-x` | `-x` | `-x` | `x` | Script returns `x` and output files not generated or expectations not reached |
| `-1000 (job.NOOUT_RC)` | `-1000` | `-0` | `0` | Script returns `0` but output files not generated or expectations not reached |
| `0` | `0` |`0`|`0`| job return code 0 with output files generated and expectations reached |
| `x` |  `x` |`x`|`x`| job return code x with output files generated and expectations reached |
| `9998 (job.EMPTY_RC)` |  `-` |`9998`|`-`| `rcfile` generated or empty |
| `9999 (job.FAILED_RC)` |  `9999` |`9999`|`-`| Failed to submit the job |

## Register your runner
It very easy to register your runner, just do `proc.registerRunner (runner_my)` (static method) before you start to run the pipeline.
